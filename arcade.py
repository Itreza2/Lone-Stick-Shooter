'''
Attention : l'auteur décourage toute lecture de ce code par un programmeur expérimenté
Si vous voulez quand même jetter un coup d'oeuil, ben c'est à vos risques et périls...
Vous êtes  prévenus !
'''
...
lProj=[]; salles=[]; grille=[]; grilleP=[]; animO=[[1,4,None]]; portal=[None, None]; debordeur=0
Combats=[]; ennemis=[]; gStats=[time(), 0]; nL=True; pression=0; angle=pi/100
pStats=[0, 100, 100, 250, 100, 100, randint(0,2)]; bonus=[1,1,0]
...
def init():
    global wStats, pStats, level, ennemis, lProj, Combats, gStats, bonus, pression
    can.delete('all')

    wStats=arsenal[0]; pStats=[0, 100, 100, 250, 100, 100, randint(0,2)]; bonus=[1,1,0]
    level=0; ennemis=[]; lProj=[]; Combats=[]; gStats=[time(), 0]; pression=0
    chargement()

def affichage():
    global perso, lProj, grilleP, pos, portal, animO, lObj, lCoffre, lDed, ded1, ded2, powers
    can.delete('all')
    ...
    if pStats[1]>0:#UI partie en cours
        ...
        can.create_text(tk.winfo_screenwidth()-275, 300, text='Niveau : '+str(level), anchor='w', fill='gainsboro', font=('Ubuntu', 16))
        ...
        can.create_text(tk.winfo_screenwidth()/2, tk.winfo_screenheight()/2-100, text='Niveau atteint : '+str(level), 
...
def main():
    ...
    global lText, tac, debordeur, lDed, gStats, powers, bonus, angle
    ...
    if pStats[1]>0:
        if lInput[0]:
            ...
            posP[0]+=(time()-frame)*(pStats[3]+bonus[2])
        if lInput[1]:
            if angle<pi/2 or angle>3*pi/2:anim=-1
            posP[0]-=(time()-frame)*(pStats[3]+bonus[2])
        if lInput[2]:posP[1]-=(time()-frame)*(pStats[3]+bonus[2])
        if lInput[3]:posP[1]+=(time()-frame)*(pStats[3]+bonus[2])
    ...
    if lInput[4] and pStats[1]>0 and powers[0][1]==False:#Tir de l'Arme (JOUEUR)
        if (time()-pStats[0])>wStats[6]:
            for i in range(int(wStats[4]*bonus[1])):
                disp=angle+(-(wStats[5]/2/180*pi)+random()*(wStats[5]/180*pi))
                ...
                if wStats[-1]==0:
                    lProj.append([
                        ...
                        disp, wStats[7], time(), True, 3*bonus[0], 'black', 'black'
                    ])
                else:
                    lProj.append([
                        pos[0]+(wStats[3])*cos(disp), pos[1]+8+(wStats[3])*sin(disp), pos[0]+(wStats[3])*cos(disp), pos[1]+8+(wStats[3])*sin(disp),
                        disp, wStats[7], time(), True, 3*bonus[0], None, None
                    ])
        ...
        if grille[int((lProj[i][1]+20)/40)][int((lProj[i][0])/40)]=='0':Holocaust.append(i)
            ...
            for j in range(len(ennemis)):
                if (ennemis[j][2]-bestiaire[ennemis[j][-1]][5]>=lProj[i][0]+2 or ennemis[j][2]+bestiaire[ennemis[j][-1]][5]<=lProj[i][0]-2 or
                    ...
                    if randint(1, wStats[9])==1:
                        if wStats[-1]==0:Holocaust.append(i)
                        ennemis[j][5]-=wStats[8]*1.5
                        lText.append([lProj[i][0], lProj[i][1], 'orange', str(-wStats[8]*1.5), time(),0.2, 14])
                    else:
                        if wStats[-1]==0:Holocaust.append(i)
                        ennemis[j][5]-=wStats[8]
                        lText.append([lProj[i][0], lProj[i][1], 'yellow', str(-wStats[8]), time(),0.2, 12])
        ...
        if Combats[ennemis[i][0]][3]:
            ...
            if (time()-ennemis[i][11])>ennemis[i][9][6]*2 and ((ennemis[i][6]==0 and ennemis[i][7]==0) or level%3==0) and debordeur<1 and ((time()-pStats[0])<wStats[6]/2 or (time()-pStats[0])>wStats[6]*2):
                ...
                for j in range(int(ennemis[i][9][4])+pression):
                    disp=ennemis[i][12]+(2-0.5*pression)*(-(ennemis[i][9][5]/2/180*pi)+random()*(ennemis[i][9][5]/180*pi))
                    pStats[0]=time(); ennemis[i][9][2]=3; lProj.append([
                        ...
                        ennemis[i][3]+8+(ennemis[i][9][3])*sin(disp), disp, ennemis[i][9][7]/(3-0.5*pression), time(), False, ennemis[i][-2], 'red', 'pink', ennemis[i][-3]])
    ...
    if time()-fpsLimiter>1/30:#Affichage
        fpsLimiter=time()
        if angle!=0:
            if angle<pi/2 or angle>3*pi/2:
                ...
                weapon=ImageTk.PhotoImage(Image.open('sprites/armes/'+wStats[1]+'/'+wStats[1]+str(int(wStats[2]))+'.png').rotate(angle*180/pi).resize((100, 100), Image.ANTIALIAS).transpose(Image.FLIP_LEFT_RIGHT))
            else:
                ...
                weapon=ImageTk.PhotoImage(Image.open('sprites/armes/'+wStats[1]+'/'+wStats[1]+str(int(wStats[2]))+'.png').rotate(180-angle*180/pi).resize((100, 100), Image.ANTIALIAS))
        animO[0][2]=ImageTk.PhotoImage(Image.open('sprites/portal/'+str(animO[0][0])+'.png').resize((100, 180)))
        for i in range(len(animE)):
        ...
        for i in range(len(ennemis)):#Sprites Armes Ennemies
            ...
            if ((ennemis[i][2])**2-pos[0]**2)!=0 and Combats[ennemis[i][0]][3]:
                if pos[0]>ennemis[i][2]:
                    ...
                    ennemis[i][10]=ImageTk.PhotoImage(Image.open('sprites/armes/'+ennemis[i][9][1]+'/'+ennemis[i][9][1]+str(int(ennemis[i][9][2]))+'.png').rotate(angle*180/pi).resize((size, size), Image.ANTIALIAS).transpose(Image.FLIP_LEFT_RIGHT))
                else:
                    angle=pi+atan(((ennemis[i][3])-pos[1])/((ennemis[i][2])-pos[0])); ennemis[i][12]=angle
                    ennemis[i][10]=ImageTk.PhotoImage(Image.open('sprites/armes/'+ennemis[i][9][1]+'/'+ennemis[i][9][1]+str(int(ennemis[i][9][2]))+'.png').rotate(180-angle*180/pi).resize((size, size), Image.ANTIALIAS))
            ...
            and ennemis[i][3]>pos[1]-tk.winfo_screenheight()/2-40 and ennemis[i][3]<pos[1]-tk.winfo_screenheight()/2+40):
                ...
                else:ennemis[i][10]=ImageTk.PhotoImage(Image.open('sprites/armes/'+ennemis[i][9][1]+'/'+ennemis[i][9][1]+str(int(ennemis[i][9][2]))+'.png').rotate(180-0*180/pi).resize((size, size), Image.ANTIALIAS).transpose(Image.FLIP_LEFT_RIGHT).transpose(Image.FLIP_TOP_BOTTOM))
        affichage()
...
def generationNiveau():
    global salles, grille, grilleP, monde, map, pos, portal, state, lObj, lCoffre, level, Combats, ennemis, lDed, nL, pression

    generation=True; lObj=[]; lCoffre=[]; ennemis=[]; Combats=[]; lDed=[]; level+=1
            ...
            if salles[j][i][0]==3 and level%3!=0:#Niveau Classique
                ...
                for o in range(rand):
                    ...
                    while boule:
                        ...
                        if grille[int((posP[1])/40)][int((posP[0])/40)]!='0':boule=False
                    rand=randint(0,2); ennemis.append([n, bestiaire[rand][0], posP[0], posP[1], bestiaire[rand][0], bestiaire[rand][1], 0, 0, bestiaire[rand][2],arsenal[bestiaire[rand][3]][:], 
                    ImageTk.PhotoImage(Image.open('sprites/armes/'+arsenal[bestiaire[rand][3]][1]+'/'+arsenal[bestiaire[rand][3]][1]+str(int(arsenal[bestiaire[rand][3]][2]))+'.png').resize((100, 100))), 
                    time(),0,arsenal[bestiaire[rand][3]][8], bestiaire[rand][4], rand])
                n+=1
...
def clavier(event):
    global lInput, pos, portal, state, lObj, wStats, pStats, lCoffre, state, nL, powers, angle
    var=event.keysym

    if var=='Right':lInput[0]=True; angle=pi/100
    if var=='Left':lInput[1]=True; angle=pi
    if var=='Up':
        lInput[2]=True
    ...
    if var=='y':
        ...
        if pos[0]>portal[0]-50 and pos[0]<portal[0]+50 and pos[1]>portal[1]-90 and pos[1]<portal[1]+90:state=False
        else:
            for i in range(len(lObj)):
                if pos[0]>lObj[i][2]-50 and pos[0]<lObj[i][2]+50 and pos[1]>lObj[i][3]-50 and pos[1]<lObj[i][3]+50:
...
def chargement():
    can.create_text(tk.winfo_screenwidth()/2, tk.winfo_screenheight()/2, text='CHARGEMENT...', font=('Ubuntu', 45), fill='white', anchor='center')
